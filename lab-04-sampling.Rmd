...
df <- read_csv(here("data", "us-ams-simple.csv")) %>%
  clean_names() %>% 
...
```{r systematic_sampling, echo=TRUE, eval=TRUE}
...
> The Population	=61.37161	=80.57007. My SRS mean = 63.01461; SD=	81.63108. My systematic sampling mean =25.95;	SD=27.19
Systematic sampling performed worse than SRS. The systematic sample mean was far lower than the population mean, and the SD was also much smaller. This likely happened because using k = 48 (one sample per day) can align with the daily cycle and repeatedly sample the same time-of-day. Since LE is much lower at night than during the day, the systematic design can bias the estimate depending on the starting point.
...
```{r cluster_sampling_days, echo=TRUE, eval=TRUE}
...
> Cluster sampling can have higher variance than SRS because observations within the same day are not independent. Nearby points in time tend to have similar values. By sampling only a few whole days, the sample can be strongly influenced by whether those days were unusually high or low.
...
```{r quasi_continuous, echo=TRUE, eval=TRUE}
...
> Quasi-continuous sampling preserves the seasonal pattern well because it samples regularly throughout the year. It also preserves the daily pattern well at an hourly step size, since the day–night peaks in LE are still clearly visible in the subsampled time series. Compared to sparse sampling, quasi-continuous sampling reduces the chance of missing important peaks/low values. In EAES, quasi-continuous sampling is preferable when the goal is to capture timing and dynamics, while reducing data volume.

---

## Exercise 4 — Compare all strategies in one table (and interpret)

```{r compare_all, echo=TRUE, eval=TRUE}
...
Quasi-continuous sampling gave the best estimate of both the population mean and SD for LE. It stayed very close to the population values, probably because it sampled regularly across the whole year and still had a large sample size. SRS also did a decent job, but it had a little more error since it only used 200 random points. Systematic sampling worked very poorly in this case because k = 48 can line up with the daily cycle and repeatedly sample the same time-of-day, which can strongly bias LE. Cluster sampling by days also underestimated the mean and SD because points within the same day are highly autocorrelated and not independent. In a real EAES field study with limited field days, I would combine stratification (by season or month) with clustering (sampling full days within each season) to get better coverage while staying realistic.
